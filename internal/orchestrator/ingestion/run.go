...
func Run(ctx context.Context, logger zerolog.Logger, client *pgmq.Client) error {
	logger.Info().Msg("Starting ingestion orchestrator")
	for {
		select {
		case <-ctx.Done():
		...
		msgs, err := client.ReadWithPoll(ctx, "ingestion_queue", 30, 1)
		if err != nil {
		...
		if err := client.Delete(ctx, "ingestion_queue", []int64{msg.ID}); err != nil {
